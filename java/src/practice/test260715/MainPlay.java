...
public class MainPlay {
	public static void main(String[] args) {
		...
		int[] userChoiceDice = dc.inputDice(sc, round);//주사위를 고르는 로직을 따로 분리 후 
		...
		while (true) {// 게임이 끝나는 조건이 만족하기 전까지 소스가 반복재생되어야 하므로 while(true)문을 작성
			...
			scr.playScore(user, userChoiceDice, true);
			scr.playScore(com, null, false);
			/*
			...
			if (scr.scoreJudgment(user, com)){//게임판정 메서드를 호출해서 점수판정이 false일 경우 break를 하지 않고 while문 반복
