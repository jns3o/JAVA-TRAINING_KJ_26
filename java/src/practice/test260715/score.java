...
public class score {// 유저가 고른 주사위 3개와 컴퓨터가 사용하는 1번 주사위를 3번 굴리는 값의 합을 구하고 출력하기 위한 클래스를 분리
	static int playUserScore(player user, rollingDice d, int[] choiceDice) {// (유저의 스코어를 구하는 메서드를 생성 후 메인에서 객체 생성했던
		...
		for (int i = 0; i < 3; i++) {// 유저가 고른 1번부터 3번 주사위까지 각각의 값을 구해줄때 n번째 주사위를 출력하기 위해 for문을 작성
			int result = rollDiceChoice(choiceDice[i], d); // 메인에서 choiceDice[i]에 저장된 유저가 고른 주사위의 맞는 주사위를
															// 'rollDiceChoice'메서드 호출을 통해
		...
		user.addScore(scoreBoard);// 만약 메인에서 playUserScore메서드를 호출하고 객체를 user객체를 호출했으면 scoreBoard의 값은 user라는 객체에
									// 저장이됨
		user.addCount();
