	...
	static void deposit(String[][] array) {
		...
		for (int i = 0; i < array.length; i++) {	
			if(array[i][0] != null && array[i][0].equals(account2)) {
					...
					System.out.println("<계좌 번호> " + array[i][0] + "\t <예금주 명> " + array[i][1] + "\t <계좌 잔액> " +  array[i][2] + "\n");
					System.out.print("이 계좌가 당신의 계좌가 맞다면 1, 아니라면 2를 입력해주세요 : ");					
					String account3 = sc.nextLine();
					System.out.println();
					try {int temAccount = Integer.parseInt(account3);
						if (temAccount == 1) {
							System.out.println("확인되었습니다. 입금하고자 하는 금액을 입력해주세요");
							System.out.println("[숫자로만 입력가능하며 입금액에 -(마이너스)는 들어갈 수 없고 입금액 앞에 0이 붙는 경우 시스템이 자동으로 빼고 계산합니다. 금액의 기준은 원화입니다](");
							System.out.print("입금액 : ");
							String pay = sc.nextLine();
							try {int tempay = Integer.parseInt(pay);
								if (tempay <= 0) {
									System.out.println("입금액에 오류가 생겼습니다. 입금액에 -는 입력 불가합니다, 재입력 해주세요");
									continue;									
								}else {
									array[i][2] = String.valueOf(Integer.parseInt(array[i][2]) + tempay);
								}
								
								System.out.println("\n입금되었습니다\n" + "\n-현재 고객님의 계좌 내역입니다-\n<계좌번호>" + array[i][0] + " <예금자 명> " + array[i][1] + " <현재 계좌액> " + array[i][2] + "\n");						
								return;
							} catch (Exception e) {
			...
			System.out.println("처음으로 되돌아가시려면 1, 입금화면으로 돌아가시려면 2를 입력해주세요 : ");
